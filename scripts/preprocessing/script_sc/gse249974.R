...
input_dir <- '~/006/data/combo/GSE249974'
out_dir <- '~/006/data/processed/srt/raw'
tmp_dir <- '~/006/tmp'

h5_files <- sort(Sys.glob(file.path(input_dir, 'GSM*_filtered_feature_bc_matrix.h5')))
if (length(h5_files) == 0) stop('No .h5 files found. Please extract GSE249974_RAW.tar first.')
...
sample_meta <- data.frame(sample_token = sample_tokens, stringsAsFactors = FALSE) %>%
  mutate(
    ...
    patient_num = str_match(sample_token, 'Patient_([0-9]+)')[, 2],
    timepoint = str_match(sample_token, '_(Pre|Post)$')[, 2]
  ) %>%
  mutate(
    sample_id = paste0(id, '_', gsm_id),
    donor_id = paste0(id, '_Patient_', patient_num),
    disease = 'icc',
    organ = 'blood',
    tissue = 'blood',
    lib = '10x_5_v2',
...
if (any(is.na(sample_meta$gsm_id) | is.na(sample_meta$patient_num) | is.na(sample_meta$timepoint))) {
  stop('Failed to parse sample metadata from file names')
}
...
report_path <- file.path(tmp_dir, 'gse249974_alignment_report.md')
lines <- c(
  '# gse249974 Alignment Report',
  '',
  ...
  '- disease = icc',
  '- organ = blood, tissue = blood',
  '- target = VEGFA;CTLA4;PDL1',
  '- timepoint = Pre/Post from sample name',
  '- treatment = Untreated (Pre), Immunotherapy (Post)',
  '- response = none',
  '- resp_standard = NA',
  "- lib = 10x_5_v2 (from GEO soft: 'scRNA-Seq 5' v2' and '10X Genomics 5’ v2 Single Cell User Guide')",
  '',
  ...
  '',
  '## 5) Evidence source',
  '- GEO soft: https://ftp.ncbi.nlm.nih.gov/geo/series/GSE249nnn/GSE249974/soft/GSE249974_family.soft.gz',
  '- Series summary reports CCA cohort and triple therapy context',
  '',
