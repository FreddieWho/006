...
input_dir <- '~/004.1/zz.CELLRANGER/TASK02'
meta_csv <- '~/004.1/zz.META/task02_raw.csv'
...
sample_meta <- standardize_task_meta(meta_csv, response_csv, id, 'PD1;Lenvatinib', 'PD1+Lenvatinib')
...
if (any(is.na(sample_meta$sample_id) | is.na(sample_meta$donor_id) | is.na(sample_meta$response))) {
  stop('Task02 metadata is incomplete after standardization')
}
...
if (any(is.na(meta$sample_id))) stop('Failed metadata join for task02')
...
report_path <- file.path(tmp_dir, 'task02_alignment_report.md')
lines <- c(
  '# task02 Alignment Report',
  '',
...
message('Saved task02 QS: ', qs_path)
message('Saved task02 H5AD: ', h5ad_path)
message('Saved task02 report: ', report_path)
